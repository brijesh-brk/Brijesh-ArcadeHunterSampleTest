...
public class ThirdPersonCharacter_edited : MonoBehaviour,IDamageable
{
    ...
    [SerializeField] float swordThrowSpeed;
    ...
    public void Move(Vector3 move, bool crouch, bool jump, bool Attack)
    {
        ...
        if (m_Animator.GetCurrentAnimatorStateInfo(0).IsName("Attack_1"))
        {
            return;
        }
        //Debug.Log("Hello");
    ...
    public void ThrowSword()
    {
        if (targetEnemy != null)
        {
            Vector3 enimyPos = targetEnemy.position;
            float t = Vector3.Distance(transform.position, enimyPos) / swordThrowSpeed;

            Transform _sword = Instantiate(swordPrefab);
            _sword.position = swordTransform.position;
            ...
            _sword.localScale = Vector3.one * .15f;
            _sword.DOMove(enimyPos, t);
            //.OnComplete(() =>
            //{
            //    //if (_sword != null)
    ...
    public void Damage(float power)
    {
        ...
        if (currentHealth <= 0 && !UIManager.Instance.gameStop)
        {
